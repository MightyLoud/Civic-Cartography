...
FULL_MANIFEST="tests/fixtures/production_batch_wa_65.yml"
WAVE_MANIFEST="build/wa-pb02-wave-d-manifest.yml"
CROSSWALK="evidence/production-batch-wa-65/selection/selection-crosswalk.json"

python -m pytest \
  tests/test_target_manifest.py \
  tests/test_production_batch_wa_65_selection.py \
  tests/test_production_batch_wa_pb02_wave.py \
...
PYTHONPATH=. python scripts/check_production_wave.py \
  ...
  --result-path build/wa-pb02-wave-d-acceptance.json
acceptance_status=$?
set -e
...
report = json.loads(Path("build/wa-pb02-wave-d-acceptance.json").read_text())
summary = report["summary"]
criteria = report["criteria"]
lines = [
    "## Production Batch 2 — Wave D",
    "",
...
Path("wa-pb02-wave-d-summary.md").write_text("\n".join(lines) + "\n")
PY

if [[ -n "${GITHUB_STEP_SUMMARY:-}" ]]; then
  cat wa-pb02-wave-d-summary.md >> "$GITHUB_STEP_SUMMARY"
fi
