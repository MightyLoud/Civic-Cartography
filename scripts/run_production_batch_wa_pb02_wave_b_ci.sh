...
WAVE_MANIFEST="build/wa-pb02-wave-b-manifest.yml"
...
PYTHONPATH=. python scripts/check_production_wave.py \
  ...
  --result-path build/wa-pb02-wave-b-acceptance.json
acceptance_status=$?
set -e
...
report = json.loads(Path("build/wa-pb02-wave-b-acceptance.json").read_text())
summary = report["summary"]
criteria = report["criteria"]
lines = [
    "## Production Batch 2 — Wave B",
    "",
...
Path("wa-pb02-wave-b-summary.md").write_text("\n".join(lines) + "\n")
PY

if [[ -n "${GITHUB_STEP_SUMMARY:-}" ]]; then
  cat wa-pb02-wave-b-summary.md >> "$GITHUB_STEP_SUMMARY"
fi
