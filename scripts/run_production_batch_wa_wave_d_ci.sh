...
WAVE_MANIFEST="build/wa-pb01-wave-d-manifest.yml"
...
PYTHONPATH=. python scripts/check_production_wave.py \
  ...
  --result-path build/wa-pb01-wave-d-acceptance.json
acceptance_status=$?
set -e
...
report = json.loads(Path("build/wa-pb01-wave-d-acceptance.json").read_text())
summary = report["summary"]
criteria = report["criteria"]
lines = [
    "## Production Batch 1 — Wave D",
    "",
...
Path("wa-pb01-wave-d-summary.md").write_text("\n".join(lines) + "\n")
PY

if [[ -n "${GITHUB_STEP_SUMMARY:-}" ]]; then
  cat wa-pb01-wave-d-summary.md >> "$GITHUB_STEP_SUMMARY"
fi
