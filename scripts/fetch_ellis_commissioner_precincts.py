...
LAYER_URL = (
    "https://maps.co.ellis.tx.us/arcgis/rest/services/Commissioner/"
    "Commissioner_Web_Map/MapServer/680"
)
                ...
                matches.append(
                    {
                        ...
                        "layer_id": 680,
                        "title": (layer.get("popupInfo") or {}).get("title"),
                    }
                )
    if len(matches) != 1:
...
def fetch_native_features() -> tuple[dict[str, Any], dict[str, Any], str]:
    ...
    for source in source_features:
        ...
        if not isinstance(attributes, dict) or not isinstance(geometry, dict):
            raise ValueError("ArcGIS feature lacks attributes or geometry")
        ...
        selected.append(
            (
                ...
                {
                    "type": "Feature",
                    "properties": normalize_source_properties(attributes),
                    "geometry": normalize_polygon_geometry(
    ...
    raw = {"type": "FeatureCollection", "features": [feature for _, feature in selected]}
    return raw, metadata, request_url
...
def main() -> int:
    ...
    raw, metadata, request_url = fetch_native_features()
    canonical = canonicalize(
        raw,
        district_field=DISTRICT_FIELD,
        jurisdiction_name="Ellis County",
        record_id_prefix="TX:county:ellis:commissioner_precinct",
        geometry_id_prefix="ellis-county-commissioner-precinct",
        source_agency="Ellis County GIS",
        layer_url=LAYER_URL,
    ...
    contract = {
        ...
        "map_service_item_id": MAP_SERVICE_ITEM_ID,
        "layer_url": LAYER_URL,
        "layer_name": metadata.get("name"),
        "district_field": DISTRICT_FIELD,
        "feature_count": 4,
        "district_ids": ["1", "2", "3", "4"],
        "web_map_contract": portal_contract,
    ...
    print("Wrote four Ellis County Commissioner precinct features from layer 680.")
    return 0
