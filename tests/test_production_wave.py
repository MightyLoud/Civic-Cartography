...
def test_wave_c_selection_is_the_frozen_third_twenty_targets() -> None:
    wave = select_production_wave(
        load_manifest(FULL_MANIFEST_PATH), "WA-PB01-C"
    )
    assert [target.target_id for target in wave.targets] == [
        f"WA-PB01-{number:03d}" for number in range(41, 61)
    ]
    assert wave.targets[0].jurisdiction_name == "Mesa"
    assert wave.targets[-1].jurisdiction_name == "Newport"
    assert {target.wave for target in wave.targets} == {"WA-PB01-C"}


def test_wave_c_acceptance_uses_the_reusable_wave_contract(
    tmp_path: Path,
) -> None:
    manifest, first, second, first_inventory, second_inventory = _reports(
        tmp_path, "WA-PB01-C"
